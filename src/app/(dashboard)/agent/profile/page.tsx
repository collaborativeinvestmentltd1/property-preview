...
import { FaUpload, FaShieldAlt } from "react-icons/fa";
...
export default function AgentProfilePage() {
  const [profile, setProfile] = useState({
    firstName: "Emeka",
    lastName: "Adebayo",
    email: "emeka@agents.com",
    phone: "080 1234 5678",
    address: "Lekki, Lagos",
    licenseNumber: "AGT123456",
  });

  const [documents, setDocuments] = useState({
    idCard: "National ID.pdf",
    agentLicense: "Agent License.pdf",
    bankStatement: "",
  });
    ...
    <div className="space-y-8">
      <header className="rounded-[2rem] bg-white p-8 shadow-sm border border-slate-200">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.3em] text-accent-200">Agent profile</p>
            <h1 className="mt-3 text-3xl font-bold text-primary-900">Manage your account and credentials</h1>
            <p className="mt-3 text-slate-600">Update your personal information, license details, and upload documents to verify your agent credentials.</p>
          </div>
          <div className="inline-flex items-center gap-2 rounded-full bg-accent-50 px-5 py-3 text-sm font-semibold text-primary-900">
            <FaShieldAlt className="text-accent-600" /> Verified agent
          </div>
        </div>
      </header>

      <Billboard title="License Renewal" message="Your agent license is valid until December 2024. Please renew your credentials before expiry to maintain uninterrupted service access." />
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <section className="xl:col-span-2 rounded-[2rem] bg-white p-8 shadow-sm border border-slate-200">
          ...
          <form onSubmit={saveProfile} className="mt-8 space-y-6">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              <label className="space-y-2">
                <span className="text-sm font-medium text-slate-700">Agent License Number</span>
                <input
        ...
        <aside className="rounded-[2rem] bg-accent-50 p-8 shadow-sm border border-accent-100">
          ...
          <div className="space-y-5">
            ...
            <label className="block rounded-3xl border border-slate-200 bg-white p-5">
              <span className="text-sm font-semibold text-slate-700">Agent License</span>
              <p className="mt-2 text-sm text-slate-500">{documents.agentLicense || "No file selected"}</p>
              <input
                type="file"
                accept=".pdf,.jpg,.png"
                onChange={(event) => handleFileChange("agentLicense", event.target.files)}
                className="mt-4 w-full text-sm text-slate-700"
              />
            </label>
            <label className="block rounded-3xl border border-slate-200 bg-white p-5">
